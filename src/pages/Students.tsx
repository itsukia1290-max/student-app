...
import SchoolCalendar from "../components/SchoolCalendar"; // ★追加
...
export default function Students() {
  ...
  const [tab, setTab] = useState<Tab>("list"); // ★追加
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (e1) {
        console.error("❌ load students:", e1.message);
      } else {
      ...
      if (e2) {
        console.error("❌ load pending:", e2.message);
      } else {
  ...
  async function approve(req: PendingReq) {
    ...
    if (!pe && prof) {
      ...
      const { data: ok2, error: e3 } = await supabase
        .from("profiles")
        .select("id, name, phone, memo")
        .eq("role", "student")
        .eq("is_approved", true)
      ...
      if (e3) {
        console.error("❌ reload students:", e3.message);
      } else {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* ★ タブ（生徒一覧 / 塾カレンダー） */}
      <div className="flex gap-2 border-b bg-white p-3 rounded-2xl">
        <button
                  ...
                  <li key={p.id} className="border rounded-lg p-3 bg-white">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => approve(p)}
                          className="btn text-sm bg-green-600"
                        >
                          承認
                        </button>
                        <button
